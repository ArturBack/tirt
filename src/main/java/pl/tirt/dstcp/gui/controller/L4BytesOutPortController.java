 ...
 */
public class L4BytesOutPortController {
    ...
    @FXML
    private ComboBox<String> sending_port_combo;
    ...
    private void initCheckBoxes() {
        sending_port_combo.getItems().clear();
        sending_port_combo.getItems().addAll(bytesCount.keySet());
        sending_port_combo.getSelectionModel().select(0);
        sending_port_combo.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
            @Override
    ...
    private void fillChartWithProperData(int startIndex) {
        String sourcePort = sending_port_combo.getSelectionModel().getSelectedItem();
    ...
    private void fillPacketValues(List<BitsInPacketInfo> data) {
        for(BitsInPacketInfo info : data) {
            ...
            if (info.getSourceIP().equals(DataUtils.HOME_IP_ADDRESS)) {
