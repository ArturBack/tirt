package pl.tirt.dstcp.data;

/**
 * Created by AWALICZE on 15.04.2017.
 */
public class DataUtils {

    public static String DIRECTORY_PATH = "";
    public static String PACKETS_FILE_NAME = "pak2.txt";
    public static String STREAM_PACKETS_FILE_NAME = "packets_to_stream.txt";
    public static String LOGGER_NAME = "logger";
    public static String HOME_IP_ADDRESS = "192.168.1.44";
    public static String HOME_MAC_ADDRESS = "a0:99:9b:1d:4a:73";

}
